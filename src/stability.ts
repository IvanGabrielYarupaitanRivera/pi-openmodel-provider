...
export const STABILITY_SUMMARY_URL = "https://api.openmodel.ai/web/v1/model-stability/summary"

/** Health status derived from success rate */
export type HealthStatus = "operational" | "healthy" | "degraded" | "unstable" | "no_data"
...
function determineHealth(successRate: number, confidence: ConfidenceLevel): HealthStatus {
...
export async function fetchModelStabilityDetail(
  ...
  if (!body.success) throw new Error(`Model stability detail request failed for ${modelKey}`)

  return {
    ...body.data,
    health_status: determineHealth(body.data.summary.success_rate, body.data.confidence),
  }
}
